...
int shoulder_r_right = 0, elbow_r_right = 0, smallleg_r_left = -10, smallleg_r_right = 10, bigleg_r_left =-10 , bigleg_r_right = 10;
int shoulder_r_left = 20, elbow_r_left = 20;
bool leftArmForward = false, rightArmForward = true;
bool leftLegForward = false, rightLegForward = true;
GLfloat robotloc = -0.5;
...
void display(void)
{
	...
	{
		modelview = modelview * glm::translate(identity, glm::vec3(robotloc, 0.0, 0.0));
		pushMatrix(modelview);
		modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)shoulder_r, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
		popMatrix(modelview);
	}
	popMatrix(modelview);
	...
	*/
	...
	pushMatrix(modelview);
	{
		...
		//================================================
			///*floor
		//================================================
			///*robot body(head+ bodyitself)
		pushMatrix(modelview);
		{
			modelview = modelview * glm::translate(identity, glm::vec3(0,0,0.67)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(0.0, 1.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(1.0, 0.0, 0.0));
			modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)shoulder_r_right, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
			glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
		...
		//cylinder
		glUniform3f(colorPos, 0.5f, 0.5f, 0.0f);
		pushMatrix(modelview);
		modelview = modelview * glm::translate(identity, glm::vec3(0.0,0.0,0.0));
		float size = 1.0f;
		modelview = modelview * glm::scale(identity,glm::vec3(1.0,1.5,3.2));
		glUniformMatrix4fv(modelviewPos,1,GL_FALSE,&(modelview)[0][0]);
		drawcylinder();
		popMatrix(modelview);
		...
		pushMatrix(modelview);
		{
			// cylinder
			pushMatrix(modelview);
			modelview = modelview * glm::translate(identity, glm::vec3(0, 0, 0));
			modelview = modelview * glm::rotate(glm::mat4(1.0f), 0 * glm::pi<float>() / 180.0f, glm::vec3(1, 0, 0.0));
			modelview = modelview * glm::rotate(glm::mat4(1.0f), 0 * glm::pi<float>() / 180.0f, glm::vec3(0, 1, 0.0));
			glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
			pushMatrix(modelview);
			modelview = modelview * glm::scale(identity, glm::vec3(1, 1, 3));
			glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
			drawcylinder();
			popMatrix(modelview);
		...
		*/
		...
		/* New for Demo; add lighting effects */
		{
			...
			pushMatrix(modelview);
			// try to let the light track on the object
		...
		pushMatrix(modelview);
		{	
			modelview = modelview * glm::translate(identity, glm::vec3(0, -0.15, 0.45)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(0.0, 1.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(1.0, 0.0, 0.0));
			{
				pushMatrix(modelview);
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)shoulder_r_right, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));

				pushMatrix(modelview);
				modelview = modelview * glm::scale(identity, glm::vec3(1.3, 0.4, 0.2));
				glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
				glUniform3f(colorPos, 0.0f, 1.0f, 1.0f);
				...
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)elbow_r_right, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));


				pushMatrix(modelview);
				modelview = modelview * glm::scale(identity, glm::vec3(1.0, 0.3, 0.2));
				glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
				glUniform3f(colorPos, 0.0f, 1.0f, 1.0f);
				//glUniform3fv(colorPos, 1, _cubecol[2]); // blue
				//drawcolor(CUBE, 1);
		...
		pushMatrix(modelview);
		{
			...
			{
				pushMatrix(modelview);
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)shoulder_r_left, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
				//glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
				...
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)elbow_r_left, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
				//glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
		...
		pushMatrix(modelview);
		{
			modelview = modelview * glm::translate(identity, glm::vec3(0, 0.2, 0.18)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(0.0, 1.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * 90, glm::vec3(1.0, 0.0, 0.0));
			{
				pushMatrix(modelview);
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)bigleg_r_left, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));

				pushMatrix(modelview);
				modelview = modelview * glm::scale(identity, glm::vec3(1.5, 0.5, 0.2));
				glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
				glUniform3f(colorPos, 0.0f, 1.0f, 1.0f);
				...
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)smallleg_r_left, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));


				pushMatrix(modelview);
				modelview = modelview * glm::scale(identity, glm::vec3(1.5, 0.4, 0.2));
				glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
				glUniform3f(colorPos, 0.0f, 1.0f, 1.0f);
		...
		pushMatrix(modelview);
		{
			...
			{
				pushMatrix(modelview);
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)bigleg_r_right, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
				...
				modelview = modelview * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0)) * glm::rotate(identity, glm::pi<float>() / 180.0f * (GLfloat)smallleg_r_right, glm::vec3(0.0, 0.0, 1.0)) * glm::translate(identity, glm::vec3(0.1, 0.0, 0.0));
...
void robotArmsAnimation() {
	int shoulder_speed = 1;
	int elbow_speed = 2;
	int smallarm_speed = 1;
	int bigarm_speed = 2;
	if (rightArmForward) {
		shoulder_r += 1;
	...
	if (shoulder_r > 15 || shoulder_r <= -15) {
		rightArmForward = !rightArmForward;
	}
	if (bigarm_r > 15 || bigarm_r <= -15) {
		rightUpForward = rightUpForward;
	}
	//movement
	robotloc += 0.005;
	if (robotloc > 0.5) {
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'p': // ** NEW ** to pause/restart animation
		animate = !animate;
		if (animate) glutIdleFunc(robotArmsAnimation);
